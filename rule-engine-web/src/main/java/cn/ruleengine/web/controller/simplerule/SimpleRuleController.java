 ...
 */
package cn.ruleengine.web.controller.simplerule;
...
@Api(tags = "规则控制器")
@RestController
@RequestMapping("ruleEngine/simpleRule")
public class SimpleRuleController {

    @Resource
    private SimpleRuleService ruleService;
    ...
    @PostMapping("list")
    @ApiOperation("规则列表")
    public PageResult<ListSimpleRuleResponse> list(@RequestBody PageRequest<ListSimpleRuleRequest> pageRequest) {
     ...
     */
    @ReSubmitLock
    @DataPermission(id = "#ruleDefinition.id", dataType = DataPermissionType.SIMPLE_RULE, type = PermissionType.VALID_WORKSPACE)
    @PostMapping("saveOrUpdateRuleDefinition")
    @ApiOperation("保存或者更新规则定义信息")
    public BaseResult saveOrUpdateRuleDefinition(@Valid @RequestBody SimpleRuleDefinition ruleDefinition) {
     ...
     * @return 规则定义信息
     */
    @DataPermission(id = "#idRequest.id", dataType = DataPermissionType.SIMPLE_RULE, type = PermissionType.VALID_WORKSPACE)
    ...
    public BaseResult getRuleDefinition(@Valid @RequestBody IdRequest idRequest) {
        PlainResult<SimpleRuleDefinition> plainResult = new PlainResult<>();
     ...
     */
    @ReSubmitLock
    @DataPermission(id = "#releaseRequest.id", dataType = DataPermissionType.SIMPLE_RULE, type = PermissionType.VALID_WORKSPACE)
     ...
     */
    @ReSubmitLock
    @DataPermission(id = "#updateRuleRequest.id", dataType = DataPermissionType.SIMPLE_RULE, type = PermissionType.VALID_WORKSPACE)
    @PostMapping("updateRule")
    @ApiOperation("更新规则信息")
    public BaseResult updateRule(@Valid @RequestBody UpdateSimpleRuleRequest updateRuleRequest) {
        PlainResult<Boolean> plainResult = new PlainResult<>();
    ...
    public BaseResult getRule(@Valid @RequestBody IdRequest idRequest) {
        PlainResult<GetSimpleRuleResponse> plainResult = new PlainResult<>();
    ...
    public BaseResult getViewRule(@Valid @RequestBody IdRequest idRequest) {
        PlainResult<ViewSimpleRuleResponse> plainResult = new PlainResult<>();
     ...
     * @return true
     */
    @DataPermission(id = "#idRequest.id", dataType = DataPermissionType.SIMPLE_RULE, type = PermissionType.DELETE)
    @PostMapping("delete")
    @ApiOperation("删除规则")
